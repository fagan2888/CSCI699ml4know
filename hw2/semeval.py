...
class SEMLoad(object):
    ...
    def __init__(self, root_path, data_type, max_len=98, limit=50, embedding_size=50):
        ...
        self.w2v_path = os.path.join(root_path, 'vectors_{}.txt'.format(embedding_size))
        self.train_path = os.path.join(root_path, 'train.txt')
        self.val_path = os.path.join(root_path, 'val.txt')
        ...
        if data_type == 'train':
            ...
            self.lexical_feature, sen_feature, self.labels = self.parse_sen(self.val_path)
        else:
    ...
    def load_w2v(self):
        ...
        wordlist.append('<pad>')
        vecs.append(np.zeros(self.embedding_size))
    ...
    def parse_sen(self, path):
        ...
        for line in open(path, 'r'):
            ...
            ent1 = (int(line[2]), int(line[3]))
            ent2 = (int(line[4]), int(line[5]))
...
if __name__ == "__main__":
    ...
    data = SEMLoad('support/', data_type='train')
    data.save()
    data = SEMLoad('support/', data_type='val')
    data.save()
    data = SEMLoad('support/', data_type='test')
    data.save()
