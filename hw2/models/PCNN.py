...
class PCNNTwoHead(PCNN):
    ...
    def __init__(self, opt):
        super(PCNNTwoHead, self).__init__(opt)
        all_filter_num = self.opt.filters_num * len(self.opt.filters)
        self.out_linear_other = nn.Linear(all_filter_num + self.opt.word_dim * 6, 2)
        self.model_name = 'PCNNTwoHead'
        self.init_model_weight_additional()

    def init_model_weight_additional(self):
        '''
        use xavier to init
        '''
        nn.init.xavier_normal_(self.out_linear_other.weight)
        nn.init.constant_(self.out_linear_other.bias, 0.)
