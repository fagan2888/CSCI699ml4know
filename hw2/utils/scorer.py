...
def evaluate(y_true, y_pred):
  ...
  """
    ...
    for rel in relation_list:
        ...
        print('{}P = {:7.2f}%\tR = {:7.2f}%\tF1 = {:7.2f}%'.format(
            header, 100 * prec[rel], 100 * recall[rel], 100 * fscore[rel]))
...
def read_test_data(true_fpath, pred_fpath):
    ...
    for line in open(true_fpath):
        ...
        y_true_dict[int(idx)] = label
    for i, line in enumerate(open(pred_fpath)):
        idx, label = line.strip().split()
        y_pred_dict[int(idx)] = normalize_label(label)
...
def normalize_label(label):
    ...
    return '-'.join(map(capitalize, label.split('-')))
